...
export async function GET({params}) {
	...
	try {
		...
		const response = await fetch(`https://ajaxorg.github.io/ace-builds/src-min-noconflict/snippets/${getAceLanguage(lang)}`)
		if (response.status == 200) {
			...
			const response = await fetch(`https://ajaxorg.github.io/ace-builds/src-min-noconflict/snippets/text.js`)
			const text = await response.text()
			// Return the file content as JavaScript
			return new Response(text, {
				headers: {
